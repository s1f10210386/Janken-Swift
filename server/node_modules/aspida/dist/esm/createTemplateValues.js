...
export default (direntTree, basePath, trailingSlash, outputMode) => {
    const imports = [];
    const pathes = [];
    const getMethodsString = (filepath, methods, indent, newPrefix, newUrl) => {
        ...
        return createMethods(methods, indent, importName, newPrefix && newPath.length > 2 ? `\`\${${newPrefix}}\${${newPath}}\`` : newPrefix || newPath, outputMode);
    };
    const createApiString = (tree, importBasePath, indent, dirDeps, prefix, url, text, methodsOfIndexTsFile) => {
        const props = tree.children
            ...
            if (dirent.isDir) {
                ...
                return fallbackSpecialCharsProp(`${createDocComment(indent, dirent.doc)}${valFn.replace('<% next %>', getMethodsString(`${importBasePath}/${basename}`, dirent.methods, `${indent}${hasVal ? '  ' : ''}`, newPrefix, newUrl))}`);
            }
            return null;
        })
            .filter((p) => !!p);
